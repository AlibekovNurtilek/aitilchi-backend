...
class Dataset:
    ...
    def __init__(self, path=None, text=None, embeddings=[], train=None, shuffle_batches=True,
        ...
        for f in range(self.FACTORS):
            self._factors.append(self.Factor(f == self.FORMS, f == self.FORMS, train._factors[f] if train else None))
        self._extras = []
                                ...
                                for part in feats_parts:
                                    if "=" in part:
                                        ...
                                        if cat in utils.FEATURE_CATEGORIES:
                                            ...
                                            feats_tuple[cat_index] = factor.words_map[cat][val]

                            factor.word_ids[-1].append(tuple(feats_tuple))
    ...
    def save_mappings(self, path):
        mappings = Dataset.__new__(Dataset)
        for field in ["_lr_allow_copy", "_variant_map", "_embeddings_size"]:
            setattr(mappings, field, getattr(self, field))
        mappings._factors = []
        for factor in self._factors:
            mappings._factors.append(mappings.Factor(factor.with_root, factor.characters, factor))

        with open(path, "wb") as mappings_file:
            pickle.dump(mappings, mappings_file, protocol=3)


    def dump_feats_debug(self, path="feats_debug.txt"):
        feats = self._factors[self.FEATS]
        with open(path, "w", encoding="utf-8") as f:
            f.write("=== FEATS WORDS MAP ===\n")
            for k, v in feats.words_map.items():
                f.write(f"{k} -> {v}\n")

            f.write("\n=== FEATS WORDS LIST ===\n")
            for i, word in enumerate(feats.words):
                f.write(f"{i}: {word}\n")

            f.write("\n=== FEATS STRINGS ===\n")
            for i, sentence in enumerate(feats.strings):
            ...
            f.write("\n=== FEATS WORD_IDS (tuple form) ===\n")
            for i, sentence in enumerate(feats.word_ids):
                f.write(f"Sentence {i}:\n")
                for t in sentence:
                    f.write(f"  {t}\n")
    ...
    def next_batch(self, batch_size, max_form_length=64):
        ...
        for f, factor in enumerate(self._factors):
            if f == self.FEATS:
                batch_word_ids.append(None)  # <-- добавляем заглушку для корректного индекса
                continue  # FEATS обработаем отдельно
        ...
        # FEATS: отдельный трехмерный тензор [B, T, 13]
        batch_feats = np.zeros([batch_size, max_sentence_len + feats_factor.with_root, feats_category_count], np.int32)
        for i in range(batch_size):
            sentence_len = batch_sentence_lens[i]
            for j in range(sentence_len + feats_factor.with_root):
                category_tuple = feats_factor.word_ids[batch_perm[i]][j]
                batch_feats[i, j, :] = category_tuple
        ...
        return self._sentence_lens[batch_perm], batch_word_ids, batch_charseq_ids, batch_charseqs, batch_charseq_lens, batch_feats



    #Тут нужно изменить для Predict
        ...
        for i in range(self._sentence_lens[index] + 1):
            ...
            for f in range(self.FACTORS):
                ...
                if overrides is not None and f < len(overrides) and overrides[f] is not None:
                    override = overrides[f][offset] if f != self.FEATS else overrides[f][i]
                    if f == self.HEAD:
                        field = str(override) if override >= 0 else "_"
                    elif (f == self.LEMMAS or f == self.XPOS or f == self.FEATS) and isinstance(override, str):
                        field = override
                    else:
                        ...
                        if f == self.LEMMAS:
                            try:
                                ...
                                print("Failed lemma rule for form '{}' and rule '{}', fallback to form".format(fields[-1], field), file=sys.stderr)
                                field = fields[-1]
    ...
    def _gen_lemma_rule(form, lemma, allow_copy):
        ...
        if not best:
            ...
            rule += "d{}¦{}".format(
                Dataset._min_edit_script(form[:best_form], lemma[:best_lemma], allow_copy),
                Dataset._min_edit_script(form[best_form + best:], lemma[best_lemma + best:], allow_copy),
            )
        return rule
