...
import tagger.dataset as dataset
import tagger.eval as eval
...
class AITilchi:
    ...
    def construct(self, args, train, devs, tests, predict_only):
        ...
        with self.session.graph.as_default():
            ...
            self.tags, self.feats_targets = {}, {}
            for tag in args.tags:
                if tag == "FEATS":
                    self.feats_targets = tf.placeholder(tf.int32, [None, None, self.NUM_OF_FEATS])
                else:
                    self.tags[tag] = tf.placeholder(tf.int32, [None, None])
            ...
            for i, category in enumerate(utils.FEATURE_CATEGORIES.keys()):
                ...
                with tf.variable_scope(f"feats_{category.lower()}"):
                    ...
                    # Дополнительные Dense-слои (если есть)
                    for _ in range(args.tag_layers):
                        feat_layer += tf.layers.dropout(
                            tf.layers.dense(feat_layer, args.rnn_cell_dim, activation=tf.nn.tanh),
                    ...
                    self.predictions_feats[category] = tf.argmax(output_layer, axis=2, output_type=tf.int32)

                    # Лосс для этой категории
                ...
                for tag in args.tags:
                    if tag == "FEATS":
                        for i, category in enumerate(utils.FEATURE_CATEGORIES.keys()):
                            ...
                            self.training_summaries.append(tf.contrib.summary.scalar(f"train/FEATS_{category}", acc))
                    else:
                        acc = tf.reduce_sum(tf.cast(tf.equal(self.tags[tag], self.predictions[tag]), tf.float32) * weights) / weights_sum
                        self.training_summaries.append(tf.contrib.summary.scalar(f"train/{tag}", acc))
    ...
    def load(self, path, morphodita_dictionary=None):
        ...
        # because it works even if TF 2 is in Eager mode.
        self.session.run(self.saver.saver_def.restore_op_name,
                        {self.saver.saver_def.filename_tensor_name: os.path.join(path, "weights")})

        # Skip consistent_feats.table since FEATS are handled as multiple softmax classifiers now.
        consistent_feats_table_path = os.path.join(path, "consistent_feats.table")
        if os.path.exists(consistent_feats_table_path):
            print("⚠️  Warning: 'consistent_feats.table' found but skipped. "
                "You are using new FEATS architecture with 13 independent classifiers.")
    ...
    def train_epoch(self, train, learning_rate, args):
        ...
        while batches < args.min_epoch_batches:
            while not train.epoch_finished():
                sentence_lens, word_ids, charseq_ids, charseqs, charseq_lens, batch_feats = train.next_batch(args.batch_size)
                ...
                for tag in args.tags:
                    if tag == "FEATS":
                        feeds[self.feats_targets] = batch_feats
                    else:
    ...
    def predict(self, dataset, evaluating, args):#Напсал чат(не проверял)
        import io
        ...
        while not dataset.epoch_finished():
            ...
            if evaluating:
                for tag in args.tags:
                    if tag != "FEATS":
                        feeds[self.tags[tag]] = word_ids[dataset.FACTORS_MAP[tag]]
                if args.parse:
            ...
            if self.morphodita:
                targets.extend([self.predictions_logits["XPOS"], self.predictions_logits["LEMMAS"]])
            if args.parse:
                targets.extend([self.heads_logs, self.deprel_hidden_layer])

            # Добавим все 14 категорий FEATS
            for category in feature_categories:
                targets.append(self.predictions_feats[category])

            predictions, *other_values = self.session.run(targets, feeds)

            if self.morphodita:
                xpos_logits, lemma_logits, *other_values = other_values
            if args.parse:
                prior_heads, deprel_hidden_layer, *other_values = other_values

            # Извлекаем предсказания по категориям FEATS
            feats_predictions = other_values[-len(feature_categories):]
            ...
            if args.parse:
                ...
                deprels = self.session.run(self.predictions_deprel,
                                        {self.is_training: False, self.deprel_hidden_layer: deprel_hidden_layer,
            ...
            for i in range(len(sentence_lens)):
                ...
                for j in range(sentence_lens[i]):
                    parts = []
                    for cat_index, category in enumerate(feature_categories):
                        value_index = feats_predictions[cat_index][i][j]
                        if value_index == 0:
                            continue  # "None"
                        value_str = feature_values[category][value_index]
                        parts.append(f"{category}={value_str}")
                    feats_override.append("_" if not parts else "|".join(parts))
                overrides[dataset.FEATS] = feats_override

                if self.morphodita:
                    self.disambiguate_with_morphodita(
                        dataset.factors[dataset.FORMS].strings[sentences][1:], dataset,
                        xpos_logits[i], lemma_logits[i], overrides)
                if args.parse:
    ...
    def postprocess_arguments(args):
        ...
        args = AITilchi.argument_parser().parse_args([args.model], namespace=args)
...
if __name__ == "__main__":
    ...
    # Parse arguments
    parser = AITilchi.argument_parser()
    ...
    # Postprocess args
    AITilchi.postprocess_arguments(args)

    # Load the data
    devs, tests = [], []
    EvaluationDataset = collections.namedtuple("EvaluationDataset", ["label", "data", "gold"])
    if not args.predict:
        train = dataset.Dataset(path=args.train, max_sentence_len=args.max_sentence_len, shuffle_batches=True,
                                               embeddings=glob.glob("{}*.npz".format(args.train)))
        #train.dump_feats_debug("feats_debug.txt")
        #sys.exit() 
        ...
        for sources, target in [(args.dev, devs), (args.test, tests)]:
            for source in sources:
                ...
                target.append(EvaluationDataset(
                    label,
                    dataset.Dataset(path=path, train=train, shuffle_batches=False,
                                                   embeddings=glob.glob("{}*.npz".format(path))),
                    eval.load_conllu_file(path, args.single_root)
                ))
    else:
        train = dataset.Dataset.load_mappings(os.path.join(args.model, "mappings.pickle"))
        test = dataset.Dataset(path=args.predict_input, train=train, shuffle_batches=False,
